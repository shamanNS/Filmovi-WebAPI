$(document).ready(function () {
	...
	$("body").on("click", "#btnfilterPoGodini", filterFilmove);

	function filterFilmove() {
		var filmoviOd = $("#godinaOD").val();
		var filmoviDo = $("#godinaDO").val();
		var zanrFilter = $("#zanrFilter").val();
		if (filmoviDo) {
			if (filmoviDo < filmoviOd) {
				alert("Godina DO ne može biti manja od godine OD!");
				//$("#btnFilterPoGodini").prop("disabled", true);
				return;
			}
		...
		// 	console.log("Filter Objekat:");
		// 	console.log(sendObject);
		var fullURL = host + port + filmoviEndpoint;
		$.getJSON(fullURL, sendObject, function (data, status) {
			console.log("poslati podaci:");
			console.log(data);
			console.log("Status: " + status);
			//updateFilmoviUI();
			if (data.length === 0) {
				//alert("Nema filmova za prikaz!");
				$("#data").empty().append("<h1>Nema filmova za prikaz!</h1>");
				$("#divAddF").css("display", "none");
				$("#divEditF").css("display", "none");
				return;
			}
			else {
				getFilmovi(data, status);

			}
			//
			// getFilmovi(data, status);
			$("#godinaOD").val(filmoviOd);
			$("#godinaDO").val(filmoviDo);
			$("#zanrFilter").val(zanrFilter);
		})
			//.done(getFilmovi(data, status))
			.fail(function () { alert("Greška pri slanju filter kriterijuma."); });
	}
	...
	function getFilmovi(data, status) {
		...
		var filterDIV = '<br /><br /><br /><div id="filterDIV"><label>Godina od: <input class="form-control" id="godinaOD" name="godinaOD"></label>&nbsp;&nbsp;';
		filterDIV += '<label>Godina do: <input class="form-control" id="godinaDO" name="godinaDO"></label>&nbsp;&nbsp;';
		filterDIV += '<label>Žanr: <input class="form-control" id="zanrFilter" name="genre"></label>&nbsp;&nbsp;';
		filterDIV += '<button id="btnfilterPoGodini" class="btn btn-info btn-md">Filtriraj</button></div><br />';
		...
		if (status == "success") {
			...
			for (var i = 0; i < data.length; i++) {
				...
				var displayData = "<tr><td>" + data[i].Naziv + "</td><td>" + data[i].Zanr + "</td><td>" + data[i].Godina + "</td><td>" + data[i].ReziserIme + " " + data[i].ReziserPrezime + "</td>";
